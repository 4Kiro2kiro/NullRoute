...
Object.assign(NODES, {
  ...
  "cheat_exploit_dev": {
    ...
    commands: [
      ...
      { label: "Structure payload finale", label_en: "Final payload structure", cmd: "# payload = \"A\"*offset + JMP_ESP_addr (little-endian) + \"\\x90\"*16 + shellcode\n# Vérifier : padding suffisant pour que le crash se produise encore", cmd_en: "# payload = \"A\"*offset + JMP_ESP_addr (little-endian) + \"\\x90\"*16 + shellcode\n# Check: enough padding so the crash still occurs" }
    ],
    lookfor: [
  ...
  "cheat_voip": {
    ...
    commands: [
      ...
      { label: "Wireshark — écouter appels VoIP", label_en: "Wireshark — intercept VoIP calls", cmd: "# Filtre : not broadcast and not multicast and host <IP>\n# Statistics → VoIP Calls → décoder en audio", cmd_en: "# Filter: not broadcast and not multicast and host <IP>\n# Statistics → VoIP Calls → decode to audio" }
    ],
    lookfor: [
  ...
  "cheat_sqli": {
    ...
    commands: [
      ...
      { label: "Nombre de colonnes (ORDER BY)", label_en: "Column count (ORDER BY)", cmd: "' ORDER BY 1--\n' ORDER BY 2--\n# Augmenter jusqu'à erreur → nb colonnes = dernier sans erreur", cmd_en: "' ORDER BY 1--\n' ORDER BY 2--\n# Increase until error → column count = last without error" },
