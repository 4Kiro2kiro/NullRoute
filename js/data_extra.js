...
Object.assign(NODES, {
  ...
  "ldap_enum": {
    ...
    commands: [
      ...
      { label: "Chercher mots de passe dans la description", label_en: "Search for passwords in description field", cmd: "ldapsearch -x -H ldap://TARGET_IP -b 'DC=domain,DC=local' '(objectClass=user)' description\n# Les admins laissent parfois le mot de passe dans la description !", cmd_en: "ldapsearch -x -H ldap://TARGET_IP -b 'DC=domain,DC=local' '(objectClass=user)' description\n# Admins sometimes leave the password in the description field!" }
    ],
    lookfor: [
...
Object.assign(NODES, {
  ...
  "web_cors": {
    ...
    commands: [
      ...
      { label: "Test avec sous-domaine arbitraire", label_en: "Test with arbitrary subdomain", cmd: "curl -s -H 'Origin: https://evil.TARGET_IP' -I http://TARGET_IP/api/\n# Si Accept-Origin: evil.TARGET_IP → vulnérable via XSS sous-domaine", cmd_en: "curl -s -H 'Origin: https://evil.TARGET_IP' -I http://TARGET_IP/api/\n# If Accept-Origin: evil.TARGET_IP → vulnerable via subdomain XSS" }
    ],
    lookfor: [
